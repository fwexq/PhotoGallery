...
#         post_obj.save()
        ...
        # if (like.model.post_id != Post.objects.get(pk=kwargs['pk']).id and like.model.user_id != request.user.id) == True:
        #     like.user = Like.objects.create(post=Post.objects.get(pk=kwargs['pk']), user_id=request.user.id)
        # else:
        #     like.post = Like.objects.filter(post=Post.objects.get(pk=kwargs['pk']), user_id=request.user.id).delete()
        # like.save()

        # if self.flag == True:
        #     Like.objects.create(post=Post.objects.get(pk=kwargs['pk']), user_id=request.user.id)
        #     self.flag = False
        #     self.save()
        #
        # else:
        #     Like.objects.filter(post=Post.objects.get(pk=kwargs['pk']), user_id=request.user.id).delete()
        #     self.flag = True
        #     self.save()
        ...
        # user = request.user
        # post_id = request.POST.get('post_obj.id')
        # if user in post_obj.liked.all():
        #     post_obj.liked.remove(user)
        # else:
        #     post_obj.liked.add(user)
        ...
        # if like:
        #     like.user = (Like.objects.create(user_id=request.user.id)).delete()
        #     like.post = Post.objects.create(pk=kwargs['pk']).delete()
        #     like.save()
...
#
#             if content.liked.filter(id=request.user.id):
#                 content.liked.remove(request.user)
#                 liked = False
#             else:
#                 content.liked.add(request.user)
#                 liked = True
#
#             ctx = {"likes_count": content.liked.all().count(), "liked": liked, "content_id": content_id}
#             return HttpResponse(json.dumps(ctx), 'main/posts/detail.html')
#
#         contents = Like.objects.all()
#         already_liked = []
#         id = request.user.id
...
#         return render(request, 'main/posts/detail.html', ctx)
